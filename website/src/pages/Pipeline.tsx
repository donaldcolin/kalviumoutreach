...
export default function Pipeline() {
    ...
    crmActivities.forEach(act => {
      ...
      if (!schoolMap[key]) {
        schoolMap[key] = {
          ...
          seminarDate: act.seminarDate || act.seminarAppointmentDate || act.followUpDate || '',
        };
      } else {
        ...
        if (act.seminarDate || act.seminarAppointmentDate || act.followUpDate) {
          schoolMap[key].seminarDate = act.seminarDate || act.seminarAppointmentDate || act.followUpDate;
        }
      }
    });
    ...
    pipelineData
      .filter(s => {
        ...
        if (dateFilter) {
          ...
          if (vDate !== dateFilter && sDate !== dateFilter) return false;
        }
        return true;
      })
      .forEach(s => {
  ...
  return (
    <div className="flex overflow-x-auto snap-x snap-mandatory h-[calc(100vh-48px)] bg-[#fafafa] text-zinc-900 animate-in fade-in duration-700 custom-scrollbar">
      <PipelineBoard 
